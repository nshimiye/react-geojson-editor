 ...
 * @param {(geojson: GeoJson<Polygon>) => void} cb used to provide access to the generated geojson object
...
export function convertPolygonToGeoJsonMulti(polygonList, context) {
    return Promise.all(
        polygonList.map(p => convertPolygonToGeoJson(p, context))
    );
}
...
export function createUIPolygon(dataPolygon, { google }) {
    ...
    const uiPolygon = new google.maps.Polygon({
        paths: [
            first.getArray(),
            // @DONE research: https://developers.google.com/maps/documentation/javascript/shapes#polygon_hole
            // @OUTCOME:
            // - Google maps shows hollow polygons if outer path is drawn in the opposite direction with the rest of the paths.
