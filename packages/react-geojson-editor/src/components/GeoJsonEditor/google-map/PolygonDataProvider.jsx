...
import {
  convertPolygonToGeoJsonMulti, convertGeojsonToPolygon,
  convertGeojsonToGMData, gmDataToPolygonList,
  createGeoJson,
} from '../../../gm-operations/convert-polygon';
 ...
 * @NOTE PolygonDataProvider is not allowed to call polygon methods (
 *      ex: polygon.setEditable or polygon.setMap)
...
export class PolygonDataProvider extends Component {
      ...
      constructor(props, context) {
        ...
        const gmapData = props.initialGeojson ?
          convertGeojsonToGMData(props.initialGeojson, context) :
          null;
        this.state = {
          gmapData,
          newPolygonList: [],
      ...
      // @TODO make sense to use getDerivedStateFromProps
      componentDidUpdate(prevProps) {
        const { initialGeojson } = this.props;

        if (prevProps.initialGeojson !== initialGeojson) {
          const gmapData = initialGeojson ?
            convertGeojsonToGMData(initialGeojson, this.context) :
            null;
          this.setState({
            gmapData,
            newPolygonList: [],
            polygonList: gmapData ? gmDataToPolygonList(gmapData, this.context) : [],
            selectedPolygonList: [],
          });
        }
      }
      ...
      subtractAction(/* ev */) {
        ...
        convertPolygonToGeoJsonMulti(this.state.selectedPolygonList, this.context)
          .then(geojsonList => subtractPolygons(geojsonList))
          .then(subtractedGeojson => convertGeojsonToPolygon(subtractedGeojson, this.context))
      ...
      deleteAction(/* ev */) {
        this.setState(state => ({
          newPolygonList: state.newPolygonList.filter(p =>
            !state.selectedPolygonList.includes(p)),
          polygonList: state.polygonList.filter(p =>
            !state.selectedPolygonList.includes(p)),
          selectedPolygonList: [],
        }));
      }
      ...
      persistChanges(_, cb) {
        ...
        // 2. send onSave action to the parent, telling it to
        //    update backend with newly created geojson
        createGeoJson([...this.state.newPolygonList, ...this.state.polygonList], this.context)
          .then(({ geojson, totalArea }) => {
            ...
            // @TODO How do i save new GeoJson and turn off edit mode?
            this.props.onSave(geojson, totalArea);
          });
      }
      // END Action handlers
      ...
      render() {
        return (
          <PolygonDataContext.Provider value={this.state}>
            {this.props.children}
...
PolygonDataProvider.propTypes = {
  children: PropTypes.node.isRequired,
  initialGeojson: GeoJsonType,
  onSave: PropTypes.func,
};

PolygonDataProvider.defaultProps = {
  initialGeojson: null,
  onSave: () => {},
};
